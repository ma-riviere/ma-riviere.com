...
abstract: |
  This post is a transposition to R of Ethan Rosenthal's [blog post](https://www.ethanrosenthal.com/2022/04/15/bayesian-rock-climbing/) on modeling Rock Climbing using a Bayesian IRT model. 
...
```{sql connection = "con_duck", time_it = TRUE, label = "sql_load", output.var = "climb_df"}
SELECT
  ascent.user_id
...
mcmc_neff_hist(neff_ratio(mod_stan))
```
...
## Summarizing draws into a list, to display the result in the same format as the `data.table` solution
(tbl(con_duck, "pp_dbp")
  |> summarize(
    .by = route_idx,
...
ridgeline_plot <- function(dat, var) {
  ...
  return(
    ...
    * ggblend::blend("multiply")
    ...
    + labs(x = str_to_title(str_replace_all(deparse(substitute(var)), "_", " ")), y = "")
...
pp_dt[climb_type == 0] |> ridgeline_plot(route_rating)
```
...
pp_dt[climb_type == 1 & bouldering_grade != "V0"] |> ridgeline_plot(bouldering_grade)
```
