...
```{r}
con_duck <- duckdb::dbConnect(duckdb(), dbdir = ":memory:")
```

```{sql connection = "con_duck", output = FALSE, output.var = "void", echo = FALSE, eval = FALSE}
INSTALL spatial;
LOAD spatial;
```
...
(tbl(con_duck, "rides")
  ...
  |> mutate(ride_dist_m = case_when(
    ...
    .default = haversine(start_lng, start_lat, end_lng, end_lat))  # This will call our SQL function
  )
...
:::: {.callout-note}
Here, `dbplyr` will leave the `decp` call as-is in the SQL translation, and since we have previously defined a `decp` SQL function, this function will get called when the SQL query is executed.
::::
