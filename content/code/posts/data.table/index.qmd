...
subtitle: "Various ways to accomplish most data wrangling operations with both `data.table` and the Tidyverse."
...
:::: {.callout-warning appearance="simple"}
`data.table` natively only supports right joins: it filters the rows of the first table by those of the second (`FIRST[SECOND]`), *but* only keeps the unmatched rows from the second table. 
