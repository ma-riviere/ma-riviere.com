...
**V5:** 2023-03-12
  ...
  - Updated code to reflect recent updates of the `Tidyverse`:  
    ...
    - `purrr` (1.0.0): `list_rbind`, `list_cbind`, `pmap_vec`, ...  
...
```{r}
mtcars |> summarize(mean_mpg = mean(mpg), sd_mpg = sd(mpg), .by = cyl)
```
...
```{r}
MT[, .(mean_mpg = mean(mpg), sd_mpg = sd(mpg)), by = cyl]
```
...
```{r}
mtcars |> 
  reframe(map_dfr(across(all_of(cols)), fn_returning_list, .id = "Var"), .by = cyl)
```
...
```{r}
mtcars |> 
  reframe(map_dfr(list_of_fns, \(f) map(across(all_of(cols)), f), .id = "Fn"), .by = cyl)
```
...
```{r}
mtcars |> 
  summarize(across(all_of(cols), list_of_fns, .names = "{col}.{fn}"), .by = cyl)
```
...
melt(FAM1, measure.vars = patterns("^dob_"))
```
...
```{r}
melt(FAM1, measure.vars = patterns(child1 = "child1$", child2 = "child2$|child3$"))
```
...
```{r}
pivot_longer(fam2, cols = matches("^dob|^gender"), names_to = c(".value", "child"), names_sep = "_child")
```
...
```{r}
melt(FAM2, measure.vars = patterns("^dob", "^gender"), value.name = c("dob", "gender"), variable.name = "child")
```
...
melt(FAM2, measure.vars = list(colA, colB), value.name = c("dob", "gender"), variable.name = "child")
```
...
melt(FAM2, measure.vars = list(a, b), value.name = c("dob", "gender"), variable.name = "child") |> 
...
```{r}
melt(FAM2, measure.vars = measure(value.name, child = \(x) as.integer(x), sep = "_child"))
```
...
melt(FAM2, measure.vars = measurev(list(value.name = NULL, child = as.integer), pattern = "(.*)_child(\\d{1})"))
```
...
dcast(FAM1L, family_id + age_mother ~ v1) |> substitute2(env = list(v1 = var_name)) |> eval()
```
...
dcast(FAM1L, v1 + v2 ~ variable) |> substitute2(env = list(v1 = id_vars[1], v2 = id_vars[2])) |> eval()
```
...
```{r}
pivot_wider(fam1l, names_from = variable, values_from = value, id_expand = TRUE, names_expand = FALSE) # (keep_id, keep_names)
```
...
Customizing the default behavior (`length()`) using the `fun.aggregate` argument:
...
```{r}
(dcast(FAM1L, family_id + age_mother ~ ., fun.agg = \(x) sum(!is.na(x))) |> setnames(".", "child_count"))
```
...
```{r}
(pivot_wider(
    DTL, id_cols = c("id1", "id2"), names_from = "group", values_from = c("v1", "v2"), 
    values_fn = \(x) tibble("sum" = sum(x), "mean" = mean(x)), names_vary = "slowest", names_sort = TRUE
  ) 
...
EVENTS[STRIKES, on = .(e.start <= s.start), nomatch = NULL
     ][, .SD[which.min(abs(e.start - e.end))], by = "event"] # No `closest` equivalent for non-equi joins in data.table
```
...
:::: {.callout-note}
`separate_wider_*` superscede both `extract` and `separate`.
::::
...
:::: {.callout-warning appearance="simple"}
`separate_longer_*` now superscedes `separate_rows`
::::
...
```{r}
DUPED |> mutate(Repeats = apply(pick(everything()), 1, \(r) r[which(duplicated(r))] |> unique() |> str_c(collapse = ", ")))
```
...
mtcars |> nest_by(cyl) |> ungroup() # Data is inside a vctrs_list_of
```
...
```{r}
mt_nest_mult |> unnest(c(data1, data2))
```
...
```{r}
mt_nest |> 
  mutate(data = map(data, \(tibl) mutate(tibl, sum = pmap_dbl(pick(everything()), sum)))) |> 
  unnest(data)
```
...
mt_nest |> 
  mutate(across(data, \(tibls) map(tibls, \(tibl) mutate(tibl, sum = apply(pick(everything()), 1, sum))))) |> 
  unnest(data)
```
