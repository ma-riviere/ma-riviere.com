---
title: "Generating predictions from a Stan model"
subtitle: "4 ways to generate predictions from a Stan model"

date: 2024-02-10

abstract: |
  See Andrew's ...https://www.andrewheiss.com/blog/2022/09/26/guide-visualizing-types-posteriors/

website:
  open-graph:
    description: "Generating predictions from a Stan model in R"
  twitter-card:
    description: "Generating predictions from a Stan model in R"
...
library(brms) # High-level interface/wrapper for Stan models
library(rstan) # (Older) R interface for Stan

library(posterior) # Wrangling Stan models' output
library(tidybayes) # Extracting and plotting from Stan models
...
set_cmdstan_path(glue::glue("{Sys.getenv('CMDSTAN_PATH')}cmdstan-{highest_cmdstan_version}"))
```
...
#| echo: false
#| output: false
#| file: !expr c(here("R", "knitr", "knit_print_ggplot_bi.R"), here("R", "knitr", "knitr_print_df_paged.R"), here("R", "knitr", "knitr_utils.R"))
```
...
left_join(
    penguins_data,
    spread_rvars(penguins_rstan_fit2, epred[ID], prediction[ID], ndraws = 500, seed = 256),
    join_by(ID)
)
```
...
penguins_predict_draws <- left_join(
    ...
    as.data.frame(penguins_predict_fit) |>
        ...
        pivot_longer(
            cols = everything(),
            names_pattern = "(epred|prediction)\\[(\\d+),(\\d+)\\]",
            names_to = c(".value", ".draw", "ID"),
            names_transform = list(ID = as.integer)
        ),
    join_by(ID)
)

(penguins_predict_draws |>
    summarize(across(contains("pred"), rvar), .by = c(ID, species, body_mass_kg_cntr, flipper_length_mm)))
```
...
```{r}
penguins_rstan_draws_samp <- spread_draws(
    penguins_rstan_fit,
    `alpha\\[\\d+\\]`,
    beta_bodymass,
    sigma,
...
penguins_r_preds <- (penguins_design_matrix |>
    ...
    unnest(cols = preds) |>
    pivot_longer(
        cols = levels(penguins_data$species),
        names_to = "species",
        values_transform = \(x) na_if(x, 0),
        values_drop_na = TRUE
    ) |>
