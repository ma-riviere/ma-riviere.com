...
# code-link (downlit) is disabled for this post: its rendered HTML exceeds 10MB, and
# xml2::read_html() on files that large silently truncates whatever token sits at the
# ~10MB mark (libxml2 buffer bug), which can shatter the page's DOM.
format:
  html:
    code-link: false
...
```{r}
copy(MT)[,
    c("min_mpg", "min_disp") := lapply(.SD, min),
    .SDcols = c("mpg", "disp")
][]
```
...
copy(MT)[, c("min_mpg", "max_mpg") := lapply(.(get(col)), \(x) list(min(x), max(x))) |> unlist(recursive = FALSE)][]
```
...
```{r}
copy(MT)[,
    c("min_mpg", "min_disp", "max_mpg", "max_disp") := lapply(.SD, \(x) list(min(x), max(x))) |>
...
```{r}
copy(MT)[,
    outer(c("min", "max"), cols, str_c, sep = "_") |> t() |> as.vector() := lapply(.SD, \(x) list(min(x), max(x))) |>
        do.call(rbind, args = _),
    .SDcols = cols
][]
```
...
```{r}
copy(MT)[, c("rmean", "rsum") := apply(.SD, 1, \(x) list(mean(x), sum(x))) |> rbindlist(), .SDcols = is.numeric][]
```
:::
...
pivot_longer(
    ...
    -ID,
    names_to = "Genre",
    values_transform = \(x) ifelse(x == 0, NA, x),
    values_drop_na = TRUE
) |>
    select(-value)
```
...
melt(FAM2, measure = list(a, b), val = c("dob", "gender"), var = "child") |>
    substitute2(env = list(a = I(str_subset(colnames(FAM2), "^dob")), b = I(str_subset(colnames(FAM2), "^gender")))) |>
    eval()
```
...
pivot_wider(
    fam2l,
    id_cols = c("family_id", "age_mother"),
...
```{r}
fam1l |>
    filter(value >= lubridate::ymd(20030101)) |>
...
```{r}
EVENTS[STRIKES, on = .(e.start == s.start), roll = "nearest"][, .SD[which.min(abs(e.start - e.end))], by = "e.id"]
```
...
```{r}
EVENTS[STRIKES, on = .(e.start == s.end), roll = -Inf][, .SD[which.min(abs(e.start - e.end))], by = "e.id"]
```
...
copy(EVENTS)[, e.start_minus2 := e.start - hours(2)][
    STRIKES,
    on = .(e.start_minus2 <= s.start, e.start >= s.start),
    nomatch = NULL
][, -"e.start_minus2"]
```
...
mutate(
    ...
    Repeats = apply(
        pick(everything()),
        1,
        \(r) r[which(duplicated(r))] |> unique() |> str_c(collapse = ", ")
    )
)
```
...
dat_agg |>
    ...
    pivot_wider(
        ...
        values_fn = \(x) ifelse(is.na(x), 0, 1),
        values_fill = 0
    )
```
...
```{r}
(melt(DAT_AGG, measure.vars = cols, variable.name = "Severity", value.name = "Count")[rep(1:.N, Count)][,
    ID_new := .I
] |>
...
DAT_AGG[Reduce(`c`, sapply(mget(cols), \(x) rep(1:.N, x)))][,
    (cols) := lapply(.SD, \(x) ifelse(x > 1, 1, x)),
    .SDcols = cols
][order(ID)]
```
:::
...
```{r}
MT_NEST_MULT[,
    do.call(c, unname(lapply(.SD, \(c) rbindlist(c)))),
    .SDcols = patterns('data'),
    keyby = .(cyl, gear)
]
```
...
MT_NEST_MULT[,
    list_cbind(unname(lapply(.SD, \(c) rbindlist(c)))),
    .SDcols = patterns('data'),
    keyby = .(cyl, gear)
]
```
...
```{r}
copy(MT_NEST)[, data := lapply(data, \(dt) dt[, sum := apply(.SD, 1, sum)])][, rbindlist(data), keyby = cyl]
```
:::
