...
# TODO:
## - ST_DistanceSphere (excepts their input to be in lat/lon EPSG:4326 (WGS84) --> see https://github.com/duckdb/duckdb-spatial/issues/450)
#### always_xy = TRUE ??
```
...
#| file: !expr here("R", "knitr", "knitr_hooks.R")
```

```{r}
#| code-fold: true
#| code-summary: "Applying a custom theme to all `gt` tables"
#| file: !expr here("R", "knitr", "knit_print_gt_mono.R")
```
:::
...
tbl(con, "rides") |>
    ...
    remote_query() |>
    bind(q, dbExecute(con, str_glue("CREATE OR REPLACE TABLE rides AS {q}")))
```
:::
...
if (!interactive()) {
    TIMES$dt_clean_geom_1 + TIMES$dt_clean_geom_2
}
```
...
(rides_clean_haversine[,
    ...
    rides_clean_geom[,
        .(
            ride_id,
            dist_geom = st_distance(
                st_transform(start_pos, st_crs(5069)),
                st_transform(end_pos, st_crs(5069)),
                by_element = TRUE
            )
        )
    ],
    ...
    labs(
        ...
        x = "Difference in distance (in meters)",
        y = ""
    ) +
...
rides_clean_geom_long <- rides_clean_geom |>
...
tbl(con, "rides_clean_geom") |>
    pivot_longer(
        cols = matches("^end|^start"),
        names_pattern = "(end|start).*_(at|name|id|lat|lng|pos)",
...
```{sql, connection="con", time_it=TRUE}
--| label: "sql_longer"
...
stations_clean_acc4_geom <- rides_clean_geom_long_acc4 |>
...
stations_clean_acc4_mean <- rides_clean_geom_long_acc4 |>
    ## Remove unknown stations
    na.omit(cols = c("id", "name")) |>
...
stations_clean_geom <- rides_clean_geom_long |>
    ## Remove unknown stations
    na.omit(cols = c("id", "name")) |>
    ## For each id + name unique combination, find the centroid of the "pos" POINT
    _[, .(pos = st_centroid(st_combine(pos))), by = .(id, name)] |>
...
stations_clean_mean <- rides_clean_geom_long |>
    ## Remove unknown stations
    na.omit(cols = c("id", "name")) |>
    ## For each id + name unique combination, average lat and lng values
...
stations_locs <- left_join(
    ...
    inner_join(tbl(con, "stations_clean_geom"), tbl(con, "stations_clean_mean"), join_by(id, name)) |>
        mutate(
            lat_unfiltered_mean = lat,
            lng_unfiltered_mean = lng,
    ...
    inner_join(tbl(con, "stations_clean_acc4_geom"), tbl(con, "stations_clean_acc4_mean"), join_by(id, name)) |>
        mutate(
            lat_filtered_mean = lat,
            lng_filtered_mean = lng,
...
matched_clean_acc4_geom <- st_join(
...
matched_clean_acc4_haversine <- fuzzyjoin::geo_inner_join(
    rides_clean_geom_long_unk_acc4,
...
if (!interactive()) {
    TIMES$dbp_match_acc4_haversine
}
```
:::
...
matched_clean_haversine <- fuzzyjoin::geo_inner_join(
...
(tbl(con, "matched_clean_haversine") |> select(name, id, lat, lng) |> distinct() |> collect())
```
...
bind_rows(
    anti_join(
        ...
        rename(any_of(c(
            name_from_geom = "name.x",
            id_from_geom = "id.x",
            name_from_haversine = "name.y",
            id_from_haversine = "id.y"
        ))) |>
        collect(),
    anti_join(
        tbl(con, "matched_clean_geom"),
        ...
        rename(any_of(c(
            name_from_geom = "name.y",
            id_from_geom = "id.y",
            name_from_haversine = "name.x",
            id_from_haversine = "id.x"
        ))) |>
        collect()
)
```
...
rides_clean_geom_long_filled <- matched_clean_acc4_geom[
    rides_clean_geom_long,
    on = setdiff(colnames(rides_clean_geom_long), c("id", "name", "pos"))
][, let(name = fcoalesce(name, i.name), id = fcoalesce(id, i.id))][,
    nms,
    env = list(nms = I(setdiff(colnames(rides_clean_geom_long), c("pos"))))
]
```
...
rides_clean_geom_filled <- rides_clean_geom_long_filled |>
...
tbl(con, "rides_clean_geom_long_filled") |>
    pivot_wider(
        ...
        values_from = c("at", "name", "id", "lat", "lng")
    ) |>
...
if (exists("duckdb_path") && file_exists(duckdb_path)) {
