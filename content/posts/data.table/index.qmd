...
**V5:** 2023-02-15
  - Grouped tabsets by framework for better readability  
  ...
  - Updated code to reflect recent updates of the `Tidyverse`:  
    - `dplyr` (1.1): .by, `reframe`, `join_by`, `consecutive_id`, ...  
    - `purrr` (1.0): `list_rbind`, `list_cbind`, `pmap_vec`, ...  
    - `tidyr` (1.3): updated the `separate_rows` section to the newer `separate_wider/longer_delim/position/regex`  
  - Updated code to reflect recent updates of `data.table` (1.14.7): `let`, `DT()`, ...  
:::
...
## TIDYR 1.3:
### Separate rows --> separate_wider/longer_*
...
### Use DuckDB included datasets, such as: CALL sqlite_attach('sakila.db');
...
FAM1L |> pivot_wider(id_cols = c(family_id, age_mother), names_from = variable, values_fn = \(x) sum(!is.na(x))) |>
  mutate(child_count = apply(select(cur_data(), matches("_child")), 1, \(x) sum(x)))
...
tidyr::extract(MT.ext, col = x, into = c("a", "b"), regex = "(.*)_(.*)", remove = F)
```
...
:::: {.callout-note}
`separate_longer_*` superscedes `separate_rows`.
::::
...
tidyr::separate_rows(SP, val, sep = ",", convert = TRUE)
```
...
```{r}
(mtcars_list <- mtcars |> summarize(mpg = list(mpg), .by = cyl))
...
```{r}
(mtcars_list2 <- mtcars |> summarize(mpg = list(mpg), .by = c(cyl, gear)))
...
```{r}
(mtcars_list_mult <- mtcars |> summarize(across(c(mpg, disp), \(c) list(c)), .by = c(cyl, gear)))
...
```{r}
(mtcars_nest_mult <- mtcars |> nest(data1 = c(mpg, hp), data2 = !c(cyl, gear, mpg, hp), .by = c(cyl, gear)))
...
```{r}
mtcars_nest_mult |> unnest(c(data1, data2))
...
```{r}
(mtcars_nest <- mtcars |> nest(data = -cyl))
...
```{r}
mtcars_nest |> mutate(sum = sum(unlist(data)), .by = cyl)
...
```{r}
mtcars_nest |> summarize(sum = sum(unlist(data)), .by = cyl)
