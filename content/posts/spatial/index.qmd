...
(tbl(con_duck, "rides_clean_l.dbp") 
  |> filter(if_all(c(lat, lng), \(x) decp(x)))
  |> compute("rides_clean_l_acc.dbp")
)
```
...
```{r}
rides_clean_l_no_coords <- (tbl(con_duck, "rides_clean_l_acc.dbp")
...
(tbl(con_duck, "rides_clean_l_acc.dbp") 
  |> filter(if_all(c(lat, lng), \(v) !is.na(v)) & (is.na(id) | is.na(name)))
  |> compute("rides_clean_l_acc_unk.dbp")
)
```
...
stations_clean[, c(k = 1, .SD)
             ...
             ][dist <= 11][, let(name = fcoalesce(name, i.name), id = fcoalesce(id, i.id))
             ][, .SD, .SDcols = colnames(rides_clean_l_acc_unk)] -> matched_clean
```
...
(fuzzyjoin::geo_join(
  ...
  |> arrange(ride_id)
  |> distinct(ride_id, way, .keep_all = TRUE)
  |> as.data.table()
)
```
...
(inner_join(
    tbl(con_duck, "rides_clean_l_acc_unk"),
    tbl(con_duck, "stations_clean"),
    sql_on = "haversine(LHS.lng, LHS.lat, RHS.lng_mean, RHS.lat_mean) <= 11"
  )
  |> mutate(name = coalesce(name.x, name.y), id = coalesce(id.x, id.y)) 
  |> select(colnames(tbl(con_duck, "rides_clean_l_acc_unk")))
  |> compute("matched_clean.dbp")
)
```
...
(st_join(
  ...
  |> mutate(name = coalesce(name.x, name.y), id = coalesce(id.x, id.y))
  |> select(colnames(rides_clean_l_acc_unk))
  |> arrange(ride_id)
  |> st_drop_geometry()
)
```
...
matched <- (furrr::future_map_dfr(
    rides_clean_l_unk[, let(ID = ceiling(.I/10000), k = 1)] |> split(by = "ID"),
    \(dat) 
      stations[, c(k = 1, .SD)][dat, on = "k", allow.cartesian = TRUE][
              , dist := haversine(lng, lat, lng_mean, lat_mean)
              ][dist <= 11
              ][, let(name = fcoalesce(name, i.name), id = fcoalesce(id, i.id))
              ][, .SD, .SDcols = setdiff(colnames(rides_clean_l_unk), c("ID", "k"))]
  )
  |> unique(by = c("ride_id", "way"))
)
```
...
SELECT
  ride_id, rideable_type, member_casual, way, ride_dur_min, ride_dist_m, ride_speed_km_h  
  , COALESCE(r.name, s.name) AS name
  , COALESCE(r.id, s.id) AS id
  , r.lat, r.lng
  , haversine(s.lng_mean, s.lat_mean, r.lng, r.lat) AS dist
...
(inner_join(
    tbl(con_duck, "rides_clean_l_unk"),
    tbl(con_duck, "stations"),
    sql_on = "haversine(LHS.lng, LHS.lat, RHS.lng_mean, RHS.lat_mean) <= 11"
  )
  |> mutate(name = coalesce(name.x, name.y), id = coalesce(id.x, id.y)) 
  |> select(colnames(tbl(con_duck, "rides_clean_l_unk")))
  |> compute("matched.dbp")
)
```
...
(st_join(
  ...
  |> mutate(name = coalesce(name.x, name.y), id = coalesce(id.x, id.y))
  |> select(colnames(rides_clean_l_unk))
  |> arrange(ride_id)
  |> st_drop_geometry()
)
```
...
(right_join(
    tbl(con_duck, "matched_clean.dbp"),
    tbl(con_duck, "rides_clean_l.dbp"),
    by = tbl(con_duck, "rides_clean_l.dbp") |> select(-name, -id) |> colnames()
  ) 
  |> mutate(name = coalesce(name.x, name.y), id = coalesce(id.x, id.y))
  |> select(-matches("\\.x|\\.y"))
  |> compute("rides_clean_l_merged.dbp")
)
```
...
pivot_wider(
  tbl(con_duck, "rides_clean_l_merged.dbp"), names_from = "way", 
