...
abstract: |
  ...
  This post is linked to a case study of **Google's Data Analytics certificate**.
...
<!-- **NB:** The SQL (DuckDB) & `dbplyr` solutions are on-disk (instead of in-memory), meaning they sacrifice some speed but could handle bigger-than-RAM datasets. -->
...
```{r}
con_duck <- dbConnect(duckdb(), dbdir = ":memory:") # duckdb_path
```
...
This solution can be faster than the pure `DuckDB` when the DB is on disk (i.e. not in `:memory:`).
...
dbRemoveTable(con_duck, "rides_arr")
...
if (file_exists(duckdb_path)) file_delete(duckdb_path)
```
