...
about:
  ...
  links:
    - text: "Article"
      icon: file-pdf
      url: https://agalic-rd.com/content/pubs/articles/CER25/
...
**2)** Helped write the methodology/statistics part of the paper ([preprint](https://www.biorxiv.org/content/10.1101/2025.10.17.683055v1)).
...
source2(
    "https://raw.githubusercontent.com/ma-riviere/DE-AoP-23/gh-pages/src/scripts/viz.R",
    NA,
    include.only = c("make_suburst_plot")
)

# Pinned to a commit: the Vasc-AoP repo's `main` was later restructured (scripts moved out of
# src/scripts/, clearing_processed.rds dropped), which breaks this post's data-loading contract.
vasc_aop_ref <- "f5f2429ec6b83ec27b12f7f9cc48d8fe970fde46"

source(str_glue("https://raw.githubusercontent.com/agalic-rd/Vasc-AoP/{vasc_aop_ref}/src/scripts/data.R"))
source(str_glue("https://raw.githubusercontent.com/agalic-rd/Vasc-AoP/{vasc_aop_ref}/src/scripts/viz.R"))
source(str_glue("https://raw.githubusercontent.com/agalic-rd/Vasc-AoP/{vasc_aop_ref}/src/scripts/utils.R"))
...
data_url_pcr <- str_glue("https://raw.githubusercontent.com/agalic-rd/Vasc-AoP/{vasc_aop_ref}/data/pcr_processed.rds")
data_url_ihc <- str_glue(
    "https://raw.githubusercontent.com/agalic-rd/Vasc-AoP/{vasc_aop_ref}/data/clearing_processed.rds"
)
data_dict_url <- str_glue(
    "https://raw.githubusercontent.com/agalic-rd/Vasc-AoP/{vasc_aop_ref}/data/__data_dictionary.xlsx"
)
gene_data_url <- str_glue("https://raw.githubusercontent.com/agalic-rd/Vasc-AoP/{vasc_aop_ref}/data/__gene_data.xlsx")

## Main

pcr_data <- readRDS(url(data_url_pcr))

# These glmmTMB models were serialized under an older glmmTMB/TMB: refresh their internal
# TMB objects so residual-based diagnostics (check_model, etc.) work under the current version.
pcr_data$models$mod <- purrr::map(pcr_data$models$mod, glmmTMB::up2date)
