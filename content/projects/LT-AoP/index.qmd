---
title: "LT-AoP"
subtitle: "Long Term effects of Apnea of Prematurity"
date: 2016-08
# doi: 

author:
  - "[DC2N (Rouen, France)](http://dc2n.labos.univ-rouen.fr/)"

image: feature.png
image-alt: "Immunohistochemistry marking of a mouse cerebellum"

description: "This project studied the impact of apnea of prematurity on cerebellar development and the long-term functional deficits resulting from it, using intermittent hypoxia in a mouse model."

abstract: |
  This study aimed to identify the effects of AoP on cerebellar development, and to demonstrate that the cerebellum is vulnerable to AoP. This was shown via enzymatic assay studies and RT-qPCR. Results confirmed the presence of alterations of cellular mechanisms, such as oxidative stress, leading to a delay in cerebellar maturation. This was further assessed through visible changes in cellular phenotype and histology via immunocytochemistry. Furthermore, behavioral studies showed that functional and behavioral alterations persisted in adulthood.

website:
  open-graph:
    image: feature.png
    description: "Impact of apnea of prematurity on cerebellar development and the long-term functional deficits resulting from it"
  twitter-card:
    image: feature.png
    description: "Impact of apnea of prematurity on cerebellar development and the long-term functional deficits resulting from it"

categories:
  - "Biostatistics"
  - "Transcriptomics"
  - "Data Science"
  - "Cerebellum"
  - "Hypoxia"
  - "RT-qPCR"

# Dirty trick to get some links/buttons
about:
  links:
    - text: "Article"
      icon: file-pdf
      url: https://cellandbioscience.biomedcentral.com/articles/10.1186/s13578-022-00869-5
      aria-label: "See a journal article illustrating the project"
    - text: "Documentation"
      icon: globe
      url: https://ma-riviere.github.io/LT-AoP-22/
      aria-label: "See the website documenting the project's data and code"
    - text: "Code"
      icon: github
      url: https://doi.org/10.5281/zenodo.6480947
      aria-label: "See the GitHub repository hosting the code of the project and related paper"
---

<hr style="margin-bottom: 30px; margin-top: -12px">

# My role in this project
***

**1)** Handle the data processing and analysis, for both immunohistochemistry and RT-qPCR data. The code for those analyses was made open-source and registered on [Zenodo](https://doi.org/10.5281/zenodo.6480947), while the results of the analyses were published in [Cell & Bioscience](https://cellandbioscience.biomedcentral.com/articles/10.1186/s13578-022-00869-5).

**2)** Make [a website](https://ma-riviere.github.io/LT-AoP-22/) documenting and showcasing the project's data, analyses, and results. The website uses Quarto and relies on templates to automatically generates documentation for each of the ~70 variables analyzed during the project.

<details>
  <summary>Click to see a preview of a page of the documentation</summary>
  <iframe src="https://ma-riviere.github.io/LT-AoP-22/quarto/content/Weight_adults.html" width="100%" height="800"></iframe>
</details>