...
get_cell_dim <- function(x) {
    ...
    if (length(dim(x) > 1)) {
        dim <- glue::glue('{dim(x)[1]} x {dim(x)[2]}')
    }
    return(dim)
}

summarize_nested_col <- \(x) {
    lapply(x, \(r) glue::glue("<{class(r)[[1]]} [{get_cell_dim(r)}]>"))
}
...
format_gt <- function(gt_tbl) {
    gt_tbl <- gt::fmt(
        ...
        columns = select(
            gt_tbl[["_data"]],
            (where(is.list) | where(\(c) "tbl_df" %in% class(c))) &
                where(\(c) all(c("sfc", "rvar") %ni% class(c)))
        ) |>
            colnames(),
    ...
    gt_tbl <- gt::fmt(
        gt_tbl,
        columns = select(gt_tbl[["_data"]], matches("p.val|^pr|pr\\(.*\\)|^p$")) |>
            colnames(),
        fns = \(x) {
            purrr::map_chr(x, \(v) {
                ifelse(
                    !is.na(v) && utils::type.convert(v, as.is = TRUE) |> is.numeric(),
                    label_pval(as.numeric(v)),
                    v
                )
            })
        }
    ...
    gt_tbl <- gt::fmt_number(
        gt_tbl,
        columns = select(gt_tbl[["_data"]], where(\(v) is.numeric(v))) |>
            colnames(),
        decimals = 3,
        sep_mark = " ",
        drop_trailing_zeros = TRUE,
        suffixing = TRUE # n_sigfig = 2
    )
    ...
    if (nrow(gt_tbl[["_data"]]) > 2) {
        gt_tbl <- tab_style(
            ...
            style = list(
                ...
                cell_fill(color = gt_strip_color, alpha = 0.6)
            ),
            locations = cells_body(rows = seq(2, nrow(gt_tbl[["_data"]]), by = 2))
        )
    }
...
style_table <- function(data, total_rows = NULL, nrows_print = 15) {
    ...
    gt::gt(data |> head(nrows_print)) |>
        ...
        bind(
            x,
            if (nrows > nrows_print) {
                x |>
                    tab_source_note(md(glue::glue(
                        "*[ omitted {scales::label_comma()(nrows - nrows_print)} entries ]*"
                    )))
            } else {
                x
            }
